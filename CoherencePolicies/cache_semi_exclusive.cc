...
void CACHE::handle_fill()
{
	...
	if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
	{
		...
		// 	As LLC is exclusive to L1 and L2, we are directly passing data to L1/L2 (bypassing L3).
		if ((cache_type == IS_LLC))
		...
		/* If it was LLC it won't reach here */


		/* 
		As it is exclusive while removing something from L2C you dont need to check whether its dirty or not
		But if cache level is L1 or L3 it needs to be dirty to send the packet below while removing
		*/
	...
	if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
	{
		...
		if(block[set][way].valid == 1){
			back_invalidate(block[set][way].address, WQ.entry[index].instr_id, this, do_fill);
				...
				/* 
				When we are in L2C while removing the victim if its a valid data then we simply send it to the lower level
				without checking whether its dirty or not since its not present in the below cache level
				But if it is any other level we check if its dirty or not, only then we would send it to lower levels.
				*/
	...
	for (uint32_t i = 0; i < MAX_READ; i++)
	{
		...
		if ((RQ.entry[RQ.head].event_cycle <= current_core_cycle[read_cpu]) && (RQ.occupancy > 0))
		{
			...
			{ // read hit
				...
				{	
					/* 
						when we read some data we move it to above cache levels so to maintain exclusiveness we will
						invalidate in the current level (remove data from the current level)
					*/
