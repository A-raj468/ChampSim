...
// This is defined here just for compiling purpose.
...
//	As L1 is exclusive to L2 and L2 is exclusive to L3 , we are directly passing data to L1. (bypassing L2 and LLC) 
#ifdef EXCLUSIVE
...
#endif
		/* If it was LLC or L2C it won't reach here */


		/* As it is exclusive while removing something you dont need to check whether its dirty or not*/
	...
	if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
	{
		...
		if(block[set][way].valid == 1){
			back_invalidate(block[set][way].address, WQ.entry[index].instr_id, this, do_fill);
				...
				/* 
				When we are in L2C or L1D or L1I while removing the victim if its a valid data then we simply send it to the lower level
				without checking whether its dirty or not since its not present in the below cache levels
				*/

				if ((cache_type == IS_L2C || cache_type == IS_L1D || cache_type == IS_L1I || block[set][way].dirty) && block[set][way].valid)
				{
