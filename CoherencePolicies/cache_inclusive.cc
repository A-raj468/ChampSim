...
/* 
In an inclusive cache, the difference with the default non-inclusive implementation
is that whenever there is an eviction in a cache level, all the lower levels need to be
evicted as well. Call this function whenever we are finding a victim.
*/
void CACHE::back_invalidate(uint64_t addr, uint32_t instr_id, MEMORY* original, uint8_t& do_fill){
	BLOCK wb_block;
	/*
	We are searching for the data in the above cache levels and if we find them dirty then we are writing the most recent modified 
	to the lower level.And making them invalid and undirty in the end.
	*/
	if(cache_type == IS_LLC){
		for(int i=0; i<NUM_CPUS; i++){
			uint32_t l1i_set = upper_level_icache[i]->upper_level_icache[i]->get_set(addr), l1i_way;
			for(l1i_way=0; l1i_way< L1I_WAY; l1i_way++){
				if(upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way].valid && 
					upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way].tag == addr){
						upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way].valid = 0;
						break;
					}
			}

			uint32_t l1d_set = upper_level_icache[i]->upper_level_dcache[i]->get_set(addr), l1d_way;
			for(l1d_way=0; l1d_way< L1D_WAY; l1d_way++){
				if(upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way].valid && 
					upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way].tag == addr){
						upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way].valid = 0;
						break;
					}
			}
			
			uint32_t l2c_set = upper_level_icache[i]->get_set(addr), l2c_way;
			for(l2c_way=0; l2c_way< L2C_WAY; l2c_way++){
				if(upper_level_icache[i]->block[l2c_set][l2c_way].valid && 
					upper_level_icache[i]->block[l2c_set][l2c_way].tag == addr){
						upper_level_icache[i]->block[l2c_set][l2c_way].valid = 0;
						break;
					}
			}

			if(l1i_way < L1I_WAY && upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way].dirty){
				wb_block = upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way];
			}
			else if(l1d_way < L1D_WAY && upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way].dirty){
				wb_block = upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way];
			}
			else if(l2c_way < L2C_WAY && upper_level_icache[i]->block[l2c_set][l2c_way].dirty){
				wb_block = upper_level_icache[i]->block[l2c_set][l2c_way];
			}

			upper_level_icache[i]->upper_level_icache[i]->block[l1i_set][l1i_way].dirty = 0;
			upper_level_icache[i]->upper_level_dcache[i]->block[l1d_set][l1d_way].dirty = 0;
			upper_level_icache[i]->block[l2c_set][l2c_way].dirty = 0;
		}
	}
	else if(cache_type == IS_L2C){
		uint32_t l1i_set = upper_level_icache[cpu]->get_set(addr), l1i_way;
		for(l1i_way=0; l1i_way< L1I_WAY; l1i_way++){
			if(upper_level_icache[cpu]->block[l1i_set][l1i_way].valid && 
				upper_level_icache[cpu]->block[l1i_set][l1i_way].tag == addr){
					upper_level_icache[cpu]->block[l1i_set][l1i_way].valid = 0;
					break;
				}
		}

		uint32_t l1d_set = upper_level_dcache[cpu]->get_set(addr), l1d_way;
		for(l1d_way=0; l1d_way< L1D_WAY; l1d_way++){
			if(upper_level_dcache[cpu]->block[l1d_set][l1d_way].valid && 
				upper_level_dcache[cpu]->block[l1d_set][l1d_way].tag == addr){
					upper_level_dcache[cpu]->block[l1d_set][l1d_way].valid = 0;
					break;
				}
		}

		if(l1i_way < L1I_WAY && upper_level_icache[cpu]->block[l1i_set][l1i_way].dirty){
			wb_block = upper_level_icache[cpu]->block[l1i_set][l1i_way];
		}
		else if(l1d_way < L1D_WAY && upper_level_dcache[cpu]->block[l1d_set][l1d_way].dirty){
			wb_block = upper_level_dcache[cpu]->block[l1d_set][l1d_way];
		}

		upper_level_icache[cpu]->block[l1i_set][l1i_way].dirty = 0;
		upper_level_dcache[cpu]->block[l1d_set][l1d_way].dirty = 0;
	}
	else if(cache_type == IS_L1D || cache_type == IS_L1I){
		...
		if(way < NUM_WAY && block[set][way].dirty){
			wb_block = block[set][way];
		}
	...
	if(wb_block.tag == 0 && block[set][way].dirty){
		wb_block = block[set][way];
	}
	if(wb_block.tag != 0){
		...
		if (lower_level)
		{
			if (lower_level->get_occupancy(2, wb_block.address) == lower_level->get_size(2, wb_block.address))
			{

				// lower level WQ is full, cannot replace this victim
				do_fill = 0;
				// lower_level->increment_WQ_FULL(wb_block.address);
				// STALL[MSHR.entry[mshr_index].type]++;

				DP(if (warmup_complete[fill_cpu]) {
									cout << "[" << NAME << "] " << __func__ << "do_fill: " << +do_fill;
									cout << " lower level wq is full!" << " fill_addr: " << hex << MSHR.entry[mshr_index].address;
									cout << " victim_addr: " << wb_block.tag << dec << endl; });
			}
			else
			{
				...
				writeback_packet.address = wb_block.address;
				writeback_packet.full_addr = wb_block.full_addr;
				writeback_packet.data = wb_block.data;
...
void CACHE::handle_fill()
{
	...
	if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
	{
		...
		if(block[set][way].valid == 1){
			back_invalidate(block[set][way].address, MSHR.entry[mshr_index].instr_id, this, do_fill);
		}
#endif
		...
		if (do_fill)
		{
			...
			sim_miss[fill_cpu][MSHR.entry[mshr_index].type]++;
			sim_access[fill_cpu][MSHR.entry[mshr_index].type]++;
			// if(MSHR.entry[mshr_index].type == WRITEBACK) assert(0);
...
void CACHE::handle_writeback()
{
	...
	if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0))
	{
		...
		{ // writeback hit (or RFO hit for L1D)
			...
			// if(WQ.entry[index].type == WRITEBACK) assert(0);
...
#ifdef INCLUSIVE
		if(block[set][way].valid == 1){
			back_invalidate(block[set][way].address, WQ.entry[index].instr_id, this, do_fill);
		}
#endif
